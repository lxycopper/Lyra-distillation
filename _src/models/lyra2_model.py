...
class Lyra2Model(WANDiffusionModel):
    ...
    def build_net(self):
        ...
        with misc.timer("Creating PyTorch model"):
            ...
            # Match WANDiffusionModel.build_net: inject LoRA on meta before FSDP (Lyra2 overrides build_net otherwise).
            if "lora_config" in config and config.lora_config.enabled:
    ...
    def inference_dmd(
        ...
        **kwargs,
        ...
        """DMD-distilled fast inference. Mirrors :meth:`inference` but uses a short
        flow-matching schedule (``num_steps`` in ``{1,2,3,4}``) and does not run CFG.
        """
        # 0) Create DMD flow scheduler; rebuild if ``num_steps`` changes (memory vs quality tradeoff).
        num_steps = int(num_steps)
        if num_steps not in (1, 2, 3, 4):
            raise ValueError(f"inference_dmd num_steps must be 1–4, got {num_steps}")
        preset_lists: dict[int, List[int]] = {
            4: [1000, 750, 500, 250],
            3: [1000, 500, 250],
            2: [1000, 250],
            1: [1000],
        }
        denoising_step_list: List[int] = preset_lists[num_steps]
        num_train_timestep: int = 1000
        if getattr(self, "_dmd_inference_num_steps_cached", None) != num_steps:
            self._dmd_inference_num_steps_cached = num_steps
        ...
        if self.dmd_scheduler is None:
            ...
            idx = torch.LongTensor(denoising_step_list)
            timesteps = torch.cat(
                (
            ...
            self.denoising_step_list = timesteps[num_train_timestep - idx]
        ...
        *,
        ...
        if cfg.spatial_memory_use_image:
            ...
            if spatial_image_ids:
                ...
                c2w_sel = torch.inverse(w2c_sel.float()).to(w2c_sel.dtype)
                #c2w_sel = torch.inverse(w2c_sel)
