#!/usr/bin/env bash
# 8-GPU launcher for train_distill_SF_dmd_lora_v1.py (DMD + LoRA with aggressive memory defaults).
#
# v1 defaults: dmd_infer_steps=2, LoRA rank=16, fp32 generator MSE, num_workers=0, self_aug off (Hydra),
# empty_cache every step. Relax via CLI flags or use train_distill_SF_dmd_lora.py instead.
#
# Usage:
#   export TEACHER_CKPT=/path/to/teacher
#   # Optional fragmentation relief:
#   # export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
#   CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 ./run_8gpus_lora_v1.sh
#
# Optional env (passed through to Python where applicable):
#   DMD_INFER_STEPS  — 1–4, default 2 in v1 script
#   LORA_RANK        — default 16
#   EMPTY_CACHE_EVERY — default 1; set 0 to disable
#   ENABLE_SELF_AUG  — set to 1 to pass --enable_self_aug
#   LOSS_FP64        — set to 1 to pass --loss_fp64
#   PRETRAINED_LORA_PATH, OUTPUT_DIR, MAX_STEPS, LR_G, LR_D, SEED, etc.
...
OUTPUT_DIR="${OUTPUT_DIR:-${LYRA2_ROOT}/outputs/distill_sf_dmd_lora_v1_fsdp_8gpu}"
...
DMD_INFER_STEPS="${DMD_INFER_STEPS:-2}"
...
exec torchrun \
  ...
  "${SCRIPT_DIR}/train_distill_SF_dmd_lora_v1.py" \
  --experiment lyra2 \
  ...
  --max_input_frames 81 
  #--train_config lyra_2/_src/train/distill_sf_dmd_config.yaml
