#!/usr/bin/env bash
# FSDP 8-GPU launcher for train_distill_SF_dmd_8gpus.py (torchrun + Lyra fully_shard).
#
# Usage:
#   export TEACHER_CKPT=/path/to/lyra2/teacher/checkpoint   # DCP parent (see model_loader) or .pth file
#   CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 ./run_8gpus.sh
#
# If CUDA_VISIBLE_DEVICES lists fewer GPUs than NPROC_PER_NODE, every rank still uses
# indices 0..NPROC-1 against the *same* visible set → multiple ranks share one GPU →
# OOM on "GPU 0" and other physical cards look idle. This script errors out in that case.
#
# Optional env:
#   NPROC_PER_NODE   (default 8)     — torchrun local processes / visible GPUs used
#   FSDP_SHARD_SIZE  (default unset) — passed as --fsdp_shard_size; omit to use WORLD_SIZE
#   MASTER_PORT      (default 29508) — avoid clash with other torchrun jobs
#   LYRA_SAMPLES, OUTPUT_DIR, MAX_STEPS, STUDENT_CKPT, CRITIC_CKPT, SEED, etc.
#   TRAIN_LORA_ONLY (default 0) — set to 1 only if the checkpoint uses PEFT LoRA (param names contain lora/adapter).
...
OUTPUT_DIR="${OUTPUT_DIR:-${LYRA2_ROOT}/outputs/distill_sf_dmd_fsdp_8gpu}"
...
if [[ -n "${CUDA_VISIBLE_DEVICES:-}" ]]; then
  ...
  if [[ "${_nvis}" -lt "${NPROC_PER_NODE}" ]]; then
    ...
    echo "torchrun gives each rank a LOCAL_RANK in 0..$((NPROC_PER_NODE - 1)); that many distinct GPUs must be visible." >&2
    echo "Fix: export CUDA_VISIBLE_DEVICES=0,1,... (at least ${NPROC_PER_NODE} indices) or unset CUDA_VISIBLE_DEVICES on an 8-GPU node." >&2
    exit 1
  fi
fi
...
exec torchrun \
  ...
  "${SCRIPT_DIR}/train_distill_SF_dmd_8gpus.py" \
  --experiment lyra2 \
