...
"""Self-Forcing DMD + LoRA — **v2 tighter peak VRAM** (same math as v1 / ``train_distill_SF_dmd_lora.py``).

v2 builds on v1 and targets OOM spikes around ``inference_dmd`` + teacher/critic ``denoise``:

- **Default** ``--dmd_infer_steps 1`` (v1 default was 2): one short-schedule DMD step → fewer activations.
- **Default** ``--empty_cache_after_prepare`` and ``--empty_cache_between_teacher_critic``: extra
  ``torch.cuda.empty_cache()`` calls to reduce fragmentation after heavy forwards (small speed cost).
- Same knobs as v1: ``--max_input_frames``, ``--lora_rank``, ``--empty_cache_every``, Hydra dataloader
  overrides, etc.

Relax quality / VRAM tradeoffs with ``--dmd_infer_steps 2..4``, ``--empty_cache_after_prepare false``,
or match v1 behavior explicitly.

Set ``PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`` in the shell when the allocator reports
fragmentation near OOM.

See also: ``run_8gpus_lora_v2.sh``.

**Temporal subsampling:** ``--max_input_frames N`` (``N>0``) uniformly subsamples the pixel time axis
before VAE encode. Keep ``N`` large enough for Lyra2 framepack.

Launch::

    torchrun --standalone --nproc_per_node=8 lyra_2/_src/train/train_distill_SF_dmd_lora_v2.py \\
...
def _maybe_uniform_subsample_input_frames_inplace(
    data_batch: dict[str, Any], model: torch.nn.Module, max_frames: int
) -> None:
    """Uniformly subsample the pixel time axis before ``get_data_and_condition`` (saves VAE / net VRAM).

    Expects batched Lyra DL3DV-style keys: ``model.input_data_key`` (B,C,T,H,W), optional ``depth``,
    ``camera_w2c``, ``intrinsics``, ``sample_frame_indices`` (B,T) aligned with T.
    """
    if max_frames <= 0:
        return
    key = getattr(model, "input_data_key", "video")
    if key not in data_batch:
        return
    vid = data_batch[key]
    if not torch.is_tensor(vid) or vid.dim() != 5:
        return
    t = int(vid.shape[2])
    if t <= max_frames:
        return
    device = vid.device
    idx = torch.linspace(0, t - 1, max_frames, device=device).round().long().clamp(0, t - 1)
    idx_cpu = idx.cpu()

    data_batch[key] = vid.index_select(2, idx).contiguous()

    si = data_batch.get("sample_frame_indices")
    if torch.is_tensor(si) and si.dim() == 2 and int(si.shape[1]) == t:
        data_batch["sample_frame_indices"] = si.index_select(1, idx_cpu.to(si.device)).contiguous()

    d = data_batch.get("depth")
    if torch.is_tensor(d):
        if d.dim() >= 2 and int(d.shape[1]) == t:
            data_batch["depth"] = d.index_select(1, idx.to(d.device)).contiguous()
        elif d.dim() >= 3 and int(d.shape[2]) == t:
            data_batch["depth"] = d.index_select(2, idx.to(d.device)).contiguous()

    cw = data_batch.get("camera_w2c")
    if torch.is_tensor(cw) and cw.dim() >= 2 and int(cw.shape[1]) == t:
        data_batch["camera_w2c"] = cw.index_select(1, idx.to(cw.device)).contiguous()

    intr = data_batch.get("intrinsics")
    if torch.is_tensor(intr) and intr.dim() >= 2 and int(intr.shape[1]) == t:
        data_batch["intrinsics"] = intr.index_select(1, idx.to(intr.device)).contiguous()

    nf = data_batch.get("num_frames")
    if torch.is_tensor(nf) and nf.dim() >= 0:
        data_batch["num_frames"] = torch.full_like(nf, int(max_frames))
    elif isinstance(nf, int):
        data_batch["num_frames"] = int(max_frames)
...
def _v2_memory_hydra_extras(args: argparse.Namespace) -> list[str]:
    """Lower host/GPU footprint: single-threaded DataLoader; optional disable self_aug."""
    # No leading ``+``: ``num_workers`` already exists on the dataloader node (``+`` is only for new keys).
    opts = [
        "dataloader_train.dataloaders.dl3dv_long_moge_chunk_81_480p_dav3_hsg.dataloader.num_workers=0",
        # PyTorch: num_workers==0 requires prefetch_factor=None (defaults use prefetch_factor=2).
        "dataloader_train.dataloaders.dl3dv_long_moge_chunk_81_480p_dav3_hsg.dataloader.prefetch_factor=null",
    ]
    if not bool(getattr(args, "enable_self_aug", False)):
        opts.append("model.config.self_aug_enabled=false")
    return opts


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Self-Forcing DMD + LoRA v2 (tighter peak VRAM, Lyra-2 FSDP)")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/distill_sf_dmd_lora_v2_fsdp")
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Default 16 (smaller adapters). Use 32+ when VRAM allows.",
    )
    parser.add_argument(
        "--lora_targets",
        type=str,
    ...
    parser.add_argument(
        ...
        default=1,
        choices=[1, 2, 3, 4],
        help="v2 default 1 (lowest peak VRAM in inference_dmd). Use 2–4 for closer-to-v1 quality.",
    )
    parser.add_argument(
        "--loss_fp64",
        action="store_true",
        help="Compute generator MSE in float64 (closer to non-v1; uses more memory).",
    )
    parser.add_argument(
        "--empty_cache_every",
        type=int,
        default=1,
        help="Call torch.cuda.empty_cache() every N steps (1=aggressive). 0 disables.",
    )
    parser.add_argument(
        "--empty_cache_after_prepare",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="After student get_data + inference_dmd (v2 default: on), empty CUDA cache before teacher/critic.",
    )
    parser.add_argument(
        "--empty_cache_between_teacher_critic",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Between teacher.denoise and critic.denoise in generator step (v2 default: on).",
    )
    parser.add_argument(
        "--enable_self_aug",
        action="store_true",
        help="Keep model.config.self_aug_enabled (default v2 turns it off via Hydra to save memory).",
    )
    parser.add_argument(
        "--max_input_frames",
        type=int,
        default=0,
        help="If >0, uniformly subsample video (and aligned depth/camera/intrinsics) to this many "
        "frames before tokenization. 0 disables. Use a value not smaller than Lyra2 framepack needs.",
    )
...
def main() -> None:
    ...
    experiment_opts_common.extend(_v2_memory_hydra_extras(args))
    experiment_opts_lora = experiment_opts_common + _lora_hydra_overrides(args)

    log.info(
        f"v2 memory: dmd_infer_steps={args.dmd_infer_steps}, loss_fp64={args.loss_fp64}, "
        f"empty_cache_every={args.empty_cache_every}, empty_cache_after_prepare={args.empty_cache_after_prepare}, "
        f"empty_cache_between_teacher_critic={args.empty_cache_between_teacher_critic}, "
        f"self_aug_hydra={args.enable_self_aug}, LoRA rank={args.lora_rank}, max_input_frames={args.max_input_frames}"
    )
    log.info(f"FSDP shard size={fsdp_shard}, world_size={world_size}")
    ...
    log.info(
        "Starting Self-Forcing DMD (FSDP + LoRA v2): "
        f"steps={args.max_steps}, fsdp_shard={fsdp_shard}, dmd_infer_steps={args.dmd_infer_steps}, "
    ...
    for step in range(1, args.max_steps + 1):
        ...
        if int(args.max_input_frames) > 0:
            _maybe_uniform_subsample_input_frames_inplace(batch, student, int(args.max_input_frames))

        _, hist, t_hist, condition, _, generated_tail = _prepare_model_inputs(
            student, batch, seed_step, dmd_infer_steps=args.dmd_infer_steps
        )

        if args.empty_cache_after_prepare:
        ...
        if train_generator:
            ...
            with torch.no_grad():
                ...
                if args.empty_cache_between_teacher_critic:
                    torch.cuda.empty_cache()
            ...
            if args.loss_fp64:
                loss_g = float(args.dmd_weight) * 0.5 * F.mse_loss(
                    generated_tail.double(), target.double(), reduction="mean"
                )
            else:
                loss_g = float(args.dmd_weight) * 0.5 * F.mse_loss(
                    generated_tail.float(), target.float(), reduction="mean"
                )
        ...
        if step % args.log_every == 0:
            ...
            log.info(
                f"[sf_dmd_lora_v2_fsdp] step={step}/{args.max_steps} "
        ...
        if step % args.save_every == 0 or step == args.max_steps:
            ...
            if distributed.is_rank0():
                ckpt_path = out_dir / f"distill_sf_dmd_lora_v2_fsdp_step_{step:07d}.pth"
                torch.save(
                    {
                        "step": step,
                        "student": student_sd,
                        "critic": critic_sd,
                        "args": vars(args),
                        "note": "FSDP LoRA v2: full gathered weights; optimizer not saved.",
                    },
                    ckpt_path,
                )
    ...
    log.info("Self-Forcing DMD distillation (FSDP + LoRA v2) finished.")
