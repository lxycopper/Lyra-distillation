...
"""Self-Forcing-style DMD distillation on 2+ GPUs using Lyra-2 FSDP (fully_shard) sharding.
...
This script:
  - Initializes NCCL via ``imaginaire.utils.distributed.init()`` (call with torchrun).
...
Launch (from Lyra-2 repo root, ``PYTHONPATH`` set as in ``run.sh``)::
    ...
    torchrun --standalone --nproc_per_node=2 lyra_2/_src/train/train_distill_SF_dmd_2gpus.py \\
...
Notes:
  ...
  - Checkpoints gather a **full** ``state_dict`` on CPU (portable .pth); optimizer states are omitted
    because gathering Adam shards is non-trivial; resume training on 2-GPU from these ckpts is limited.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Self-Forcing DMD trainer (Lyra-2 FSDP multi-GPU)")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/distill_sf_dmd_fsdp")
...
def main() -> None:
    ...
    if world_size < 2:
        raise SystemExit(
            "This entrypoint expects distributed launch with at least 2 processes.\n"
    ...
    # Sharded models (each spans ``fsdp_shard`` GPUs; typical 2-GPU job uses fsdp_shard=2).
                ...
                torch.save(
                    {
                        ...
                        "critic": critic_sd,
                        "args": vars(args),
                        "note": "FSDP2 run: optimizer states not saved; student/critic are gathered full weights.",
                    },
                    ckpt_path,
                )
