...
"""Self-Forcing DMD + LoRA — **v1 memory-saver** (same math as ``train_distill_SF_dmd_lora.py``).

Defaults trade quality / peak VRAM for fitting on tight GPUs. Relax by:

- ``--dmd_infer_steps 4`` (was 2): more DMD denoise steps in ``inference_dmd``.
- ``--loss_fp64``: MSE in float64 like the non-v1 script (uses more VRAM).
- ``--lora_rank 32`` (v1 default 16): larger adapters.
- ``--empty_cache_every 0``: stop ``torch.cuda.empty_cache`` between steps.
- ``--enable_self_aug``: re-enable Lyra2 self-augmentation forwards.
- DataLoader: v1 forces ``num_workers=0`` (less host RAM / fewer pinned buffers); override via
  extra ``--experiment_opt`` if you need workers.

See also: ``run_8gpus_lora_v1.sh`` and ``PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True``.

**Temporal subsampling:** ``--max_input_frames N`` (``N>0``) uniformly picks ``N`` frames along the
dataloader time axis before VAE encode (reduces VRAM). Keep ``N`` large enough for Lyra2 framepack
(roughly on the order of the dataset ``sample_n_frames``).

Launch::

    torchrun --standalone --nproc_per_node=8 lyra_2/_src/train/train_distill_SF_dmd_lora_v1.py \\
...
def _v1_memory_hydra_extras(args: argparse.Namespace) -> list[str]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Self-Forcing DMD + LoRA v1 (memory-saver, Lyra-2 FSDP)")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/distill_sf_dmd_lora_v1_fsdp")
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="v1 default 16 (smaller adapters). Use 32+ when VRAM allows.",
    )
    parser.add_argument(
        "--lora_targets",
        type=str,
    ...
    parser.add_argument(
        ...
        default=2,
        choices=[1, 2, 3, 4],
        help="Shorter DMD sampling in student tail generation (fewer denoise steps → less VRAM).",
    )
    parser.add_argument(
        "--loss_fp64",
        action="store_true",
    ...
    parser.add_argument(
        "--enable_self_aug",
        action="store_true",
        help="Keep model.config.self_aug_enabled (default v1 turns it off via Hydra to save memory).",
    )
    parser.add_argument(
        "--max_input_frames",
        type=int,
...
def main() -> None:
    ...
    experiment_opts_common.extend(_v1_memory_hydra_extras(args))
    experiment_opts_lora = experiment_opts_common + _lora_hydra_overrides(args)

    log.info(
        f"v1 memory: dmd_infer_steps={args.dmd_infer_steps}, loss_fp64={args.loss_fp64}, "
        f"empty_cache_every={args.empty_cache_every}, self_aug_hydra={args.enable_self_aug}, "
        f"LoRA rank={args.lora_rank}, max_input_frames={args.max_input_frames}"
    )
    ...
    log.info(
        "Starting Self-Forcing DMD (FSDP + LoRA v1): "
    ...
    for step in range(1, args.max_steps + 1):
        ...
        if step % args.log_every == 0:
            ...
            log.info(
                f"[sf_dmd_lora_v1_fsdp] step={step}/{args.max_steps} "
        ...
        if step % args.save_every == 0 or step == args.max_steps:
            ...
            if distributed.is_rank0():
                ckpt_path = out_dir / f"distill_sf_dmd_lora_v1_fsdp_step_{step:07d}.pth"
                torch.save(
                    {
                        "step": step,
                        "student": student_sd,
                        "critic": critic_sd,
                        "args": vars(args),
                        "note": "FSDP LoRA v1: full gathered weights; optimizer not saved.",
                    },
                    ckpt_path,
                )
    ...
    log.info("Self-Forcing DMD distillation (FSDP + LoRA v1) finished.")
